...
const Home: React.FC = () => {
  ...
  const getCategoryStyle = (cat: string) => {
    switch (cat) {
      ...
      case '法律咨询':
        return 'bg-amber-50 dark:bg-amber-900/30 text-amber-600 dark:text-amber-400 border-amber-100 dark:border-amber-700/50';
      case '安全教育':
      ...
      default:
        return 'bg-slate-50 dark:bg-slate-800 text-slate-500 dark:text-slate-400 border-slate-100 dark:border-slate-800';
    }
  };
  ...
  }, [selectedCategory, likedIds]);

  return (
    <div className="animate-in fade-in duration-500 dark:bg-slate-950 min-h-screen">
      <header className="px-5 py-3 bg-white/90 dark:bg-slate-900/90 sticky top-0 z-40 backdrop-blur-md flex justify-between items-center border-b border-slate-100 dark:border-slate-800 transition-colors">
        <div className="flex items-center gap-2">
          <div className="w-9 h-9 bg-orange-400 rounded-xl flex items-center justify-center text-white shadow-sm shadow-orange-200 dark:shadow-none">
        ...
        <div className="flex gap-2">
          ...
          <button onClick={() => navigate('/notifications')} className="p-2 rounded-full bg-slate-50 dark:bg-slate-800 text-slate-600 dark:text-slate-300 transition relative">
      ...
      <div ref={categoryContainerRef} className="bg-background-light dark:bg-slate-950 px-5 py-4 overflow-x-auto no-scrollbar flex items-center gap-2 sticky top-[68px] z-30 backdrop-blur-sm scroll-smooth snap-x border-b border-transparent transition-colors">
        {categoriesList.map((cat) => (
          <button
            ...
            className={`px-4 py-2.5 rounded-2xl font-black text-[13px] whitespace-nowrap transition-all border shrink-0 snap-center ${
              ...
              ? 'bg-slate-900 dark:bg-white text-white dark:text-slate-900 border-slate-900 dark:border-white shadow-xl dark:shadow-none scale-105 active-category-btn' 
              : 'bg-white dark:bg-slate-900 text-slate-400 dark:text-slate-500 border-slate-100 dark:border-slate-800 hover:bg-slate-50 dark:hover:bg-slate-800 active:scale-95'
            }`}
          >
            ...
            <div className="flex flex-wrap gap-1.5 mb-4">
              {post.categories.map((cat, idx) => (
                <span key={idx} className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-lg text-[10px] font-black border shadow-sm ${getCategoryStyle(cat)}`}>
                  {cat === '法律咨询' && <span className="material-symbols-outlined text-[12px]">gavel</span>}
                  {cat}
                </span>
              ))}
            </div>
            <div className="flex items-center justify-between pt-4 border-t border-slate-50 dark:border-slate-800">
              <div className="flex items-center gap-2.5" onClick={(e) => { e.stopPropagation(); navigate(`/user/${post.author.name}`); }}>
